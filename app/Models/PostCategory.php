...
class PostCategory extends Model
{
    ...
    Public function getTmbImgPathAttribute()
    {
        ...
        if ($this->thmb_img != '') {
            ...
            $src = 'storage/images/blog_category/' . $this->id . '/' . htmlspecialchars(strip_tags($this->tmb_img));
        } else {
            //если есть картинка вакансии
            $src = $defaultTmb;
        }
    ...
    {
        return $this->hasMany('App\Models\PostCategoryTitle', 'category_id', 'id');
    }

    public function localisedCategoryTitle()
    {
        return $this->hasOne('App\Models\PostCategoryTitle', 'category_id', 'id')
            ->where('locale_id', LocaleMiddleware::getLocaleId());
    }

    public function categoryDescriptions()
    {
        return $this->hasMany('App\Models\PostCategoryDescription', 'category_id', 'id');
    }

    public function localisedCategoryDescription()
    {
        return $this->hasOne('App\Models\PostCategoryDescription', 'category_id', 'id')
            ->where('locale_id', LocaleMiddleware::getLocaleId());
    }
    ...
    {
        return $this->belongsToMany('App\Models\Post')->using('App\Pivots\BlogCategory');
    }
